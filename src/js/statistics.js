...
const impLista = () =>
  (document.getElementById("outputDatos").innerHTML = lista);
/** Imprime en el ID 'outputDatos' del HTML el contenido de la lista */
const impCalculo = (output, calculo) =>
  (document.getElementById(output).innerHTML = calculo);
/**
...
const calcularModa = () => {
   ...
   */
  const fcl = (a, b) =>
    lista.filter((e) => e === a).length - lista.filter((e) => e === b).length;
 ...
 */
const calcularTodo = () =>
  calcularPromedio() && calcularMediana() && calcularModa() && impLista();
